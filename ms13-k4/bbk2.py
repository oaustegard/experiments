...
import os
PROBE = int(os.environ.get('MS13_PROBE', '1'))
...
def prove(rows, v, verbose_every=20000, max_nodes=None):
    ...
    def dfs(A, b, remaining, point):
        ...
        cands = sorted(remaining, key=n_ok)[:PROBE]
        best_zi, best_children = None, None
        for zi in cands:
            z = ZS[zi]
            children = []
            for a, s in opts:
                ...
                if val - SHIFT <= 0:
                    st["pruned"] += 1; continue
                children.append((A2, b2, val, x))
                if best_children is not None and len(children) >= len(best_children):
                    break
            if best_children is None or len(children) < len(best_children):
                best_zi, best_children = zi, children
                if not children:
                    break
        zi = best_zi
        rest = [x for x in remaining if x != zi]
        for A2, b2, val, x in best_children:
            if not rest:
