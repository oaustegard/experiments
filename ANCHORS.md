...
| RTX 5090 memory bandwidth | 1,792 GB/s (32 GB GDDR7) | NVIDIA spec, via secondary summary | the card, exactly | **The one genuinely hard anchor in this experiment.** Bandwidth ÷ weight bytes is a *ceiling* on single-stream decode that no tuning beats — which is what makes it useful: it does not tell you what a deployment achieves, it tells you when a quoted figure must be using a different mechanism (batching, speculation, multi-token prediction). It cannot see *which* of those, and it says nothing about prefill. | `luna-onprem-tco/hourly.py::roofline`, `recheck.py` phase 2b |
| RTX 5090 street price, US | median **$4,699.99** (Aug 2026); in-stock range $3,900–5,000; MSRP $1,999 unobtainable, AIB floor $2,900 | videocardprices.com / bestvaluegpu, via secondary summary | one month, US retail, new cards | **A spot price in a shortage, not a trend.** It is also the input with the largest leverage on `hourly.py`'s conclusion — the street-vs-MSRP gap roughly **doubles** the break-even duty cycle — so a stale value moves the answer more than any physical constant here does. No used-market or depreciation coverage. | `luna-onprem-tco/params.json:hardware`, `recheck.py` phase 5 |
