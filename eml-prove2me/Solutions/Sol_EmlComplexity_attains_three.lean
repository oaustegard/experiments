...
theorem solution : Attains ((3 : ℝ)) 14 := by
  ...
  have : (0:ℝ) < Real.exp 1 - 1 - 1 := by first | positivity | linarith | (ring_nf; linarith) | nlinarith [Real.exp_pos 1]
  have : (0:ℝ) < Real.exp (Real.exp 1 - (Real.log (Real.exp 1 - 1 - 1))) := by first | positivity | linarith | (ring_nf; linarith) | nlinarith [Real.exp_pos 1]
  have : (0:ℝ) < Real.exp (Real.exp 1 - 1 - 1 - 1) := by first | positivity | linarith | (ring_nf; linarith) | nlinarith [Real.exp_pos 1]
  refine ⟨(.node .one (.node (.node (.node .one (.node (.node .one (.node (.node .one (.node (.node .one (.node .one (.node .one .one))) .one)) (.node .one .one))) .one)) (.node .one .one)) .one)), ?_, rfl, ?_⟩
