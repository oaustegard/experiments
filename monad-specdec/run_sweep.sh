...
for S in 4 8 "$N"; do
  ...
  python3 -u eagle_train2.py "$S" 4 "s${S}" 2>&1 | grep -viE "loading weights|clean_up|attentioninterface"
  log "=== finished $S shards (rc=${PIPESTATUS[0]}) ==="
done
log "=== end-to-end with the ${N}-shard head ==="
python3 -u eagle_e2e2.py "/tmp/specdec/eagle_head_s${N}.pt" 2>&1 | grep -viE "loading weights|clean_up|attentioninterface|deprecated"
log "SWEEP COMPLETE"
