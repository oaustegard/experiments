# VENDORED from oaustegard/experiments nl2sh-instantiate/prompts.py
# sha256[:16] = 0fc41997bfbcf859
# Do not edit here. Edit the original and re-vendor with vendor.sh, which
# re-pins this hash. The pin exists because every number in
# nl2sh-instantiate/RESULTS.md was produced by these exact bytes: a silent
# drift here would keep working and quietly stop matching the measurements.
...
# Vendored: extract_params sits beside this file rather than in a sibling
# experiment directory, so the package is importable off a pip install.
