...
### `sparseup-tag-probe/` — SPARSEUP's vocabulary dimensions against Muninn's human tags

Oskar's 2024 idea was a curated binary tag vector: a few hundred domain tag
dimensions, Hamming similarity, ordered so a prefix is a coarser taxonomy. A
SPLADE-style encoder is the trained version of the first step with the
vocabulary fixed to the tokenizer. Muninn's memory store is a human-tagged corpus,
so the question — do the encoder's dimensions map onto the tag vocabulary — has
a direct test. Untrained: at k = 32, a memory's tags are among its expansion
terms 16–24% of the time depending on how a hyphenated tag is counted. Trained:
a linear probe on the sparse vector reaches micro-AP 0.612, word + char TF-IDF
on the same text 0.664, and the gap survives giving TF-IDF the same 512-token
window (−0.044 [−0.051, −0.038]). Binarizing the vector costs 0.021. The
pre-registered F1@0.5 put SPARSEUP first; under 1% of probability cells cross
0.5 for the sparse arms, so it ranked calibration. Two of five logged errors
changed the reading and both were caught by structure — one by reading the
first table against the prediction, one by the adversarial pass the plan
scheduled. Round 2 put the same probe on the 2024 idea's own first layer, a
binary vector over noun phrases selected by frequency: 0.369 micro-AP at 512
dims, 0.481 with every phrase, below word unigrams at every size and never
near the encoder. The map step builds in minutes; it does not replace the
n-gram matrix.
Round 3 asked the retrieval question instead: against 466 `refs` links, a
binary vector over the whole 5,827-tag inventory ties gte-small at R@10
(0.667 vs 0.648), survives a recency control that drops nearest-in-time
from 0.593 to 0.122, and fused with gte beats either. PMI expansion of the
tag vector never helps. TF-IDF leads every single representation, partly
because refs are written after a lexical recall.
Round 4 had flash-lite re-tag 300 memories: 0.33 with a frequent-tag hint, 0.46
asked for specific names, against 0.70 for the original tags, whose retrieval
sits in the rare tags that name projects, issues and people.
Round 5 gave flash-lite the tags of five older neighbours as write-time recall
context: 0.565, half the remaining gap, and a no-model copy of the neighbours'
tags does as well at k = 10.
Round 6 put the cited memories' own tags in the prompt: 0.629 forward and
0.617 on the reverse direction the context cannot leak into, level with
gte-small and 0.06 under the original tags. A cheap tagger with the refs'
tags in hand is within reach of the frontier-written ones.

### `modernbert-bidirectionality/` — where a bidirectional encoder reads its right context, and which cut to train

Handoff from the claude.ai session (2026-09-20); the spec body was lost in the
memory write and the design was reconstructed from the title and pre-registered
in `PLAN.md`. **Ablation** (masked-LM CE at 9,728 masked positions, same masks
in every arm): on ModernBERT-base each of layers 1–21 is individually redundant
(≤ 0.14 nats causal) and layer 0 is not (9.4 nats, a collapse that survives
making the sinks readable); keeping only the 8 global layers bidirectional
retains a third of the benefit; an 8-token lookahead in every layer retains
79%; the two special tokens alone are worse than nothing; punctuation ties the
content-free masked positions at equal budget. The pre-registered "no right
context" reference (all layers causal) was off-distribution at 23.5 nats;
`truncate` (tokens `0..i` + `[SEP]`, bidirectional) is the reference that holds.
**Adaptation** (2026-09-21, Oskar: *"Run the ettin-decoder-32m adaptation
test"*): five masks trained from the causal twin for 6M tokens each. Global
layers bidirectional and the rest causal: 2.945 nats; everything bidirectional:
2.996; 8-token lookahead everywhere: 3.164; causal control: 4.181. The
ablation's recommendation (lookahead everywhere) is withdrawn; the dQwen3.5
cut holds. One of seven ablation predictions right, two of six adaptation
predictions right, and METHODS.md carries the lesson about ablation floors.

### `spd-hungarian-decoder/` — Hungarian decoding versus sorting on arXiv:2609.01807

arXiv:2609.01807 decodes a ranking as the optimal bipartite assignment of an
N×K item-position score matrix. The paper ablates the head, the training signal
and the backbone, and never the decoder. This holds the matrix fixed and swaps
the solver for four cheaper decoders, on real learning-to-rank data with the
paper's own head and Sinkhorn objective.

The diagnostics come before the metrics because they decide whether the
comparison means anything: the assignment constraint binds on 100% of slates
(35.9 of 50 items collide on row-argmax), so the solver is not idle, and the
matrix is 80.9% rank-1, so most of what it solves is a sort. `PLAN.md` states
what each reading would be if the paper were entirely right, and was committed
before the first model trained.
...
late-interaction head — so the question became how remex and remax compare to
those on the model's own terms, on SciFact with real qrels.

The late-interaction head carries the model. At fp32 it scores 0.720 nDCG@10
against the dense head's 0.553, at 40x the bytes (164 KB/doc vs 4 KB). Coding
each 128-d token at 1 bit closes that byte gap to 5.1 KB/doc and keeps 0.707.
Token pooling, the lever Sentence Transformers ships for this head, costs more
quality for less saving: halving the tokens costs 0.019, coding them at 2 bits
saves 16x for 0.013.

The dense head reverses this repo's "quantize wide rather than truncate narrow"
rule. NeoMME was trained at the truncation dims, so 1024→256 costs 0.015 while
4→1 bits at full width costs 0.045; 4-bit at the dim the budget allows wins from
64 B to 512 B, and at 512 B it matches fp32. The earlier rule compared bits
against fp32 truncation on encoders where truncation was expensive; both are
right about their encoders, and the bit ladder still has to be measured per
model.

The remaining piece is a calibration: remex 1-bit and remax k=1 with a float
query are the same code under a different rotation seed, and they land 0.006
and 0.012 apart on the two heads. That is the noise floor for any single-seed
comparison here.

Second corpus, ViDoRe DocVQA: 500 scanned pages encoded as images, no OCR,
about 2,900 patch vectors each. The token head keeps its lead over the dense
head (0.519 against 0.385), the 1-bit token index is within 0.004 of fp32 at
a 32x saving, and pooling, which cost real points on text, is free on pages
(−0.004 at half the tokens), so the two stack: half the tokens at one bit is
23 KB per page, 64x under fp32, for −0.004. The one SciFact finding that does
not carry over is the strict 4-bit-narrow-over-2-bit-wide ordering on the
dense head; on DocVQA the 128–512 B budget is a tie within noise.

Third corpus, ShiftProject (1,000 report pages, 100 generated queries, an
easy set): the token ladder loses nothing at any bit width, 1-bit tokens sit
+0.016 over fp32 with the interval spanning zero, and the SciFact dense
ordering comes back — 4-bit at 256 dims beats 1-bit at 1,024 by 0.040, with
the interval clear of zero. Across the three: 1-bit tokens are within noise
of fp32 at 32x on text and on pages; 4-bit dense at full width matches fp32
everywhere; 1-bit dense at full width loses everywhere.
