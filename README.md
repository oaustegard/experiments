...
|---|---|---|---|---|
| [`lattice-representation-hypothesis/`](lattice-representation-hypothesis/RESULTS.md) | 2026-07-31 | done (thesis refuted by own adversarial arm; one reversal survives) | [`RESULTS.md`](lattice-representation-hypothesis/RESULTS.md) + [`THEORY.md`](lattice-representation-hypothesis/THEORY.md) + [`fca.py`](lattice-representation-hypothesis/src/fca.py) + `noise_reversal.png` | A *Paper Skygest* Bluesky post pointing at [arXiv:2603.01227, "The Lattice Representation Hypothesis of LLMs"](https://arxiv.org/abs/2603.01227) (LLM embedding geometry encodes an FCA concept lattice; meet/join as half-space operations). Hypothesis under test: the concept algebra has a **broken join** — FCA's meet extent is a bare intersection (exact under half-spaces) while its join extent is the *closure* of a union, and the paper's Definition 7 writes that join as a literal set union, contradicting its own Appendix B. Measured at scale: **0 meet phantoms across 9,615,370 concept pairs**, join overshoot mean 0.60 (3.1x extent inflation). **Then the experiment's own adversarial arm killed most of it.** Definition 7's "conic hull" clause makes the join *exactly* `R(Y_A n Y_B)` whenever the attribute directions are linearly independent — which holds throughout the paper (|M| <= 184, d >= 3072); verified 96/96 by an independently written Minkowski-sum LP, strict only under conic dependence (43/96, 34/96). And the "phantoms" are not errors: they *are* the least upper bound, and in the WordNet task they are the gold label (join of {dog, wolf} is `canine`, which contains foxes). **The surviving result is a reversal of the opening thesis**: on WordNet contexts with real MiniLM embeddings and held-out probes (AUC 0.87-0.98, controls at chance), injecting probe error shows the **join is more robust than the meet at every noise level** (tree, 5% flip: join J=0.939 vs meet J=0.835; cross: 0.875 vs 0.753) — the closure absorbs probe noise, the bare intersection compounds it. Two further survivors: **hypernym trees are the worst case, not a benign one** (0.93 non-closed / 0.71 overshoot, worse than iid controls — refuting the "hierarchical data is nested so it's fine" defence; a chain gives 0.000), and **Eq. 6's meet orientation looks inverted** (max beats min in 96.5% of trials) though the join half failed to replicate (59%, cosines tied) against the reviewing agent's claimed 99.5%. Forensics from two independently-prompted readers: Figure 4's numbers are printed nowhere in the paper (recovered identically to 3 d.p. from its SVG bar geometry), **12 of 45 Table 2 cells have F1 below both their own precision and recall** (arithmetically impossible for a harmonic mean), and no released code computes Figure 4 at all. Figure 4's join deficit is 5/5 domains and survives a Random-baseline difficulty control, but the Mean baseline shows a comparable excess, so it is **unexplained, not attributed**. Negative results kept in the record: Arm C's meet metric was vacuous by construction (my bug) and its probes were indistinguishable from random directions. |
