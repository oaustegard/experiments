...
|---|---|---|---|---|
| [`ttt-embed-quantized/`](ttt-embed-quantized/RESULTS.md) | 2026-08-14 | **done — artifact committed; fp32 nDCG@10 = 0.7067** | [`RESULTS.md`](ttt-embed-quantized/RESULTS.md) + [`data/`](ttt-embed-quantized/data/) + [`encode_scifact.py`](ttt-embed-quantized/encode_scifact.py) + [`recheck.py`](ttt-embed-quantized/recheck.py) + [`ERRORS.md`](ttt-embed-quantized/ERRORS.md) | [#33](https://github.com/oaustegard/experiments/issues/33) — a *data product*, not a finding: encode BEIR SciFact once with jina-v5-nano q4 so claude.ai never has to. claude.ai measures this encoder at **<2 docs/s on one core** and reaps detached jobs after ~100 s, which caps an in-session encode at ~200 of 5,183 documents. Committed: `Dm.npy` (5183, 256) fp32, `Q.npy` (300, 256), `meta.json`. Settings pinned to the 2026-07-08 codec eval so its fidelity numbers carry over (dim=256 Matryoshka, max_length=384, `title + ". " + text`, `Document: `/`Query: ` prefixes, last-token pool, L2, fp32). **Sanity check passes: fp32 nDCG@10 of `Q @ Dm.T` = 0.7067**, inside the pre-registered 0.60–0.72 band. **Two forced deviations, both verified rather than assumed.** (1) **Every** HF host is 403 at this container's egress proxy — not just the `us.aws.cdn.hf.co` leg #33 warns about but `huggingface.co`, `hf.co`, `datasets-server`, `cdn-lfs` and **both** CDN legs — and a policy denial must not be retried, so the corpus is rebuilt from the upstream AllenAI release BEIR itself wraps (`scifact.s3-us-west-2.amazonaws.com`, reachable). Two mappings there are not guessable: **BEIR's *test* split is AllenAI's *dev* claims** (AllenAI's test claims ship unlabelled), and **relevance is `cited_doc_ids`, not `evidence`** — `evidence` is the SUPPORT/CONTRADICT subset and yields 209 pairs over 188 queries against BEIR's published 339 over 300. `verify_shape()` asserts all four published counts (5183/1109/300/339) before a single vector is computed and all four match; hitting **339 not 340** is load-bearing, since one dev claim cites the same document twice. (2) The mirror's `scripts/embed_onnx.py` **cannot load the q4 asset** — its `materialize()` hardcodes the 847 MB fp32 `model.onnx` — so its pooling/prefix/normalize semantics are replicated against the same `tokenizer.json` and `pad_id=128001`. **Measured, not argued: mini-batching is bit-exact** (batch 32 vs batch 4, max\|Δ\| = **0.000e+00** across all 256 dims), upgrading `q4-official-vs-ours`' first-principles claim and making batch size a pure throughput knob. **The speed story is not what it looks like:** 4.4 docs/s on 4 vCPU is ~1.1 docs/s/core against claude.ai's <2 on one — *no better per core*, only ~2.2× whole-machine. The job is possible here because 20 minutes of wall-clock is available, not because the cores are faster, so throwing cores at the next encode will not help much. **Known gap, registered in `ANCHORS.md`:** the count check pins the *split*, not the document *strings*, and the nDCG band is far too loose to notice a few points lost to a wrong `title`/`abstract` join — a per-document diff against the real `BeIR/scifact` is the missing check and no reachable host serves it. Next (elsewhere): learn the TTT-Embed task vector under an oracle teacher and sweep the index through remex 8/4/2/1-bit and remax k=8/4/2. |
...
### `ttt-embed-quantized/` — SciFact encoded once, so claude.ai never has to

A data product rather than a result. The deliverable is three files —
`data/Dm.npy` (5183, 256) fp32, `data/Q.npy` (300, 256), `data/meta.json` —
plus the script that made them, committed so the downstream TTT-Embed ×
remex/remax sweep can run in numpy in seconds wherever it likes.

The sanity check passed on the first clean run: **fp32 nDCG@10 of `Q @ Dm.T`
against the qrels = 0.7067**, inside the 0.60–0.72 band issue #33 pre-registered
for SciFact with a small retrieval encoder.

Three things are worth carrying elsewhere.

**A blocked host list is a property of the session, not of the dataset.** Issue
#33 said to retry HF's LFS redirect until it lands on the allowlisted GCP CDN
leg. That is true of claude.ai and false here, where *every* HF host answers 403
at the egress proxy. The recovery generalises: BEIR-style datasets are
re-derivable from the upstream benchmark they wrap, and those upstreams are
ordinary S3/GitHub hosts. What does **not** generalise is the mapping — BEIR's
test split is AllenAI's *dev* claims, and qrels come from `cited_doc_ids`, not
the smaller `evidence` field. Guessing `evidence` would have produced 209 pairs
over 188 queries instead of 339 over 300, inflated nDCG by shrinking the
denominator, and **still landed inside the sanity band**. The check that caught
it was the benchmark's own published counts, asserted before any compute.

**Measure the invariance you are relying on; it costs one batch.**
`q4-official-vs-ours` mini-batched to dodge a ~26 GB attention-mask `Expand`
OOM and argued from first principles that per-row output is unchanged. Encoding
32 documents at batch 32 and at batch 4 gives max|Δ| = **0.000e+00** — exactly
zero, not merely small. Two lines of code turned an argument into a fact.

**Moving a job off claude.ai buys wall-clock, not throughput.** 4.4 docs/s on 4
vCPU is ~1.1 docs/s per core against claude.ai's <2 on one. The corpus encode is
only ~2.2× faster whole-machine and still takes 20 minutes. What made it
possible is that claude.ai reaps detached jobs after ~100 s — capping any
in-session encode near 200 documents regardless of core speed. Diagnose the
constraint before sizing the box for it.
