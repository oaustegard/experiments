...
| [`latent-calculator/`](latent-calculator/RESULTS.md) | 2026-09-06 | **done — a frozen SmolLM2-135M reads an exact calculator result injected into its residual stream at layer 16 on 0.60 of prompts (Monad 0.39), categorical results at 1.00 and numbers at a rate falling with digit count; neither model exposes operand digits linearly at the query token at any depth, so the learned query drops SmolLM2 to 0.30** | [`RESULTS.md`](latent-calculator/RESULTS.md) + `PREDICTIONS.md` + `data.py` + `model_utils.py` + `probe.py` + `query_head.py` + `train_port.py` + `eval.py` + `run_all.sh` + `test_latent_calculator.py` + `results/` | Oskar, the depth-3 port from the in-model-tools design space (memory 88477132): can a tool go between layers, latent in and latent out, for an exact result rather than a retrieval. Frozen Pleias Monad 56.7M and SmolLM2-135M; trained parts under 1.1M params (query head, result encoder); residual add, single KV slot, and delayed-by-one injection arms; text-token and frozen baselines; oracle vs learned query. Residual add beats the KV slot on both models (as in 2603.22329); the text route never lands the bare answer at this size (exact 0.000, contains 0.19-0.39); delayed injection is free when the first answer token is a space and blind when it carries the answer. D1, D3 confirmed; D2, D5 refuted; D4 refuted on the held-out length because consumption fails first. 21 tests, ruff clean, resumable driver, 5.4 h CPU. |
