...
**Part B upholds the incumbent.** Official jina v5 nano q4 wins 11 of 12 iso-byte
cells against bekko, including by 0.03–0.10 R@10 on the code distribution where
bekko was supposed to have its edge — at 131.6 MB against bekko-a8m's 124.1 MB,
so there is no size argument either. No remax_kb swap. A side effect worth
recording: the `7cecfd94` "regime A" collapse (ship the encoder with the `.kb`,
retire the endpoint-and-secret) was **already available with the incumbent** and
did not need this benchmark.
