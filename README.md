...
|---|---|---|---|---|
| [`needle-esp32-budget/`](needle-esp32-budget/README.md) | 2026-08-20 | **done — the bytes almost fit and the arithmetic does not; ~50 GFLOP per routing turn** | [`README.md`](needle-esp32-budget/README.md) + `results_kv_*.json` | Oskar: "the goal is to be able to run this on an esp32 with 16 MB of memory". Prices the three preceding experiments against a real budget (**16 MB of PSRAM**, confirmed). **The file was never the problem**: the engine reports `peak_ram_mb` on every completion — a field `eval.py` drops — and twelve real routing turns measure **43.8 MB peak** on the stock weights, 43.9 MB process RSS, against 14.3 MB of Python interpreter, so **~29.5 MB of engine against 13.74 MB of weights**. The `weights=` path costs ~13 MB more again (the blob resident more than once; RSS tracks ~2.75x each weight delta). At 11.16–12.36 MB the model file fits 16 MB with room; the working set is ~2.7x the budget. **And the two size reductions do not compose**: `[9,13)` pruned alone is 0.574, `default=2` alone is 0.630, both together are **0.389** at 11.16 MB — a −0.037 and a +0.018 composing to −0.222, which neither experiment could have predicted from its own arm. **Weights**: 12.36 MB floor at no accuracy cost (`needle-quantization`), ~11.2 MB if you also take the one surviving depth cut and its 3.7 points. **KV**: Cactus's own `per_pos` formula gives **16,704 bytes per cached position**, so the pinned 256 window is **4.28 MB** — 16.64 MB resident before activations, tokenizer, grammar, engine or app. **And the window is not a knob**: `write_export(kv_window=...)` writes the header field, but exporting at 8/160/192/224/256/384/512 and scoring all seven gives **one distinct outcome** (0.611/0.613/0.625/0.537 at every window) — an 8-token window cannot serve a 481-token prompt, so the field is recorded and ignored; `export.py` calls it the width the model was *trained* with. **The real wall is compute, not memory**: the 5 rendered schemas are **467 tokens**, re-prefilled every turn with no prefix cache, and 481 prefill tokens ÷ 423.9 tok/s = 1.13 s against a measured 1.21 s turn — so the turn *is* prefill, at **~50 GFLOP per routing decision** (43.5 matmul + 6.4 attention). Against plausible ESP32 int8 throughput that is tens of seconds to minutes per query, and a dense model re-reads all 12.36 MB per decoded token against tens of MB/s of PSRAM. Also: `cactus-needle`'s `agent/fetch.py` builds wheels for manylinux/macOS/Windows only — the `.cact` is portable, the engine as distributed is not. If the 16 MB is flash, weights fit with ~3.6 MB left and **no room for an OTA slot**. **The lever left is not in the model**: prefill dominates because 467 tokens of schema are re-read every turn, and `monad-bsky`'s **regex-only baseline routes this catalogue at 0.833 in 0.022 ms** — more accurately than the model, at zero compute, with 15.9 MB to spare. Hardware figures are inferred from training data, not measured, and flagged as needing verification against the specific part. |
| [`needle-quantization/`](needle-quantization/RESULTS.md) | 2026-08-20 | **done — negative: the model is already at the knee; the one move left is 10% of the file, free** | [`RESULTS.md`](needle-quantization/RESULTS.md) + [`PREREG.md`](needle-quantization/PREREG.md) + [`ERRORS.md`](needle-quantization/ERRORS.md) + [`params.json`](needle-quantization/params.json) + [`recheck.py`](needle-quantization/recheck.py) + `results_*.json` | Oskar, after `needle-layer-pruning`: "so we can't trim the layer depth. Can we quantize?". The premise needed correcting first — **Needle 2 already ships aggressively quantized**, `weight_bits = "embedding=4,mhc=4,default=2"`, so the bulk is at **2 bits** with two tensor families protected at 4 and the 13.74 MB blob is that spec. Ten arms over the exporter's whole range (`CQ_BITS = (2, 3, 4)` plus ternary), same 62 queries and scorer as the three siblings; `shipped` reproduces **0.611** exactly, as every control in this series has. **Bit width does not matter above 2**: the eight arms with no ternary in the bulk span **0.593–0.648 — 3.0 queries of 54 — while their blobs span 12.36 to 23.17 MB, and zero of the eight differ significantly from shipped.** A 1.9x swing in file size buys nothing either way. **No headroom above**: `all4` at **+68.6% bytes** scores 0.593, nominally the worst of them (falsifies P2, which predicted +0.00 to +0.06) — the checkpoint's own `weight_bits` and the exporter's `qapt` stage say why, these weights were made quantization-aware for this spec. **The 4-bit protection is not load-bearing, which inverts P3**: `all2` is **12.36 MB (−10.0%)** at **0.630**, p=1.00 — smaller than shipped and not worse; split apart, `emb2` (−7.6%) lands exactly on shipped and `mhc2` (−2.4%) at 0.630. **Below 2 bits there is nothing to get, by construction**: `_packed_row_bytes` returns `in_pad * 2 // 8` for ternary and `_pack_ternary_crumbs` writes **2-bit crumbs**, so `TERNARY_BITS = 1.58` is log2(3) — the information content, not the storage width — and ternary occupies exactly the bytes of `bits=2` with three codebook levels instead of four. Measured: `prot-tern` is byte-identical to shipped and costs **39 points** (0.222, p<0.0001), `all-tern` costs **57** (0.037) and **refuses all 62 queries** (refusal 1.000). **Third witness for the Engram result**: ternarizing both 4.19M n-gram tables — 18.6% of the model — scores **0.648**, two queries *above* shipped at unchanged bytes, agreeing with `needle-layer-pruning` (destroying a whole table costs less than four well-placed layers) and `needle-tool-naming` (routing lives in selection over provided context, not recall). Answer: quantizing further is effectively done; take `default=2` for 10% off and nothing else is on the table. Errors: a ternary size estimate wrong by 3.5x from reading the constant's name instead of the packer, caught by exporting one probe before designing the sweep; and four arms scored on a loaded box, so **no latency number is reported at all** rather than a caveated table. Three of seven predictions held. |
| [`needle-layer-pruning/`](needle-layer-pruning/RESULTS.md) | 2026-08-20 | **done — negative: a 45M model trained to 27 layers has almost no slack; 1 of 24 cuts survives, and the standard heuristic cannot find it** | [`RESULTS.md`](needle-layer-pruning/RESULTS.md) + [`PREREG.md`](needle-layer-pruning/PREREG.md) + [`ERRORS.md`](needle-layer-pruning/ERRORS.md) + [`params.json`](needle-layer-pruning/params.json) + [`recheck.py`](needle-layer-pruning/recheck.py) + `results_*.json` | Oskar, after `needle-depth-growth`: "the opposite: can we prune layers from needle and still retain functionality?". Unlike growth this needs no training — delete, re-export, measure. Cuts are contiguous blocks of 4 because that is what preserves the MHC lane assignment (`i % 4`); `engram_layers=(2, 15)` is absolute, so 7 of the 24 arms also destroy a 4.19M-parameter n-gram table. Every arm is scored against an **unpruned export** (`Needle(weights=...)` reports `confidence: None`, so the gate is out of scope), and that control reproduces `needle-bsky`'s `tuned-min` and `needle-tool-naming`'s `canon` at **0.611** on five separate runs — which validates the export path. **One position of twenty-four survives**: `[9,13)` at **0.574**, Δ −0.037, paired McNemar **p=0.81** (10 vs 8 discordant) — a genuine null. Twenty of twenty-four are significant at p<0.05 and **five leave a model that refuses every query**. **The redundant depth is in the middle, not the deep end**, falsifying the pre-registered prediction: the tolerant zone is starts 7–14, while `[20,24)`, `[21,25)` and `[22,26)` are three of the five worst — the inverse of the large-decoder result (Gromov et al. 2024, ShortGPT). **The cheap heuristic fails outright**: angular distance across the block correlates with measured accuracy at **Spearman +0.061** over 24 blocks; its top pick `[20,24)` measures 0.018 and the actual best ranks 10th. The six lowest-distance blocks span a 0.016 band containing both the 3rd-best position (`[7,11)`, 0.481) and the two worst (0.000) — those late layers barely move the residual stream and are still load-bearing, so the 24 evaluations were not avoidable. **Engram loss is not the dominant term, inverting another prediction**: `[12,16)` and `[13,17)` each destroy an entire 9.3%-of-model n-gram table *and* four layers and rank 2nd and 4th (0.500, 0.463), while `[21,25)` destroys no table and scores 0.000 — the memory subsystem is worth less than four correctly-chosen layers. Deeper cuts collapse: 8 layers over the tolerant zone tops out at **0.148**, 12 layers hits **0.000** and refuses everything. **And it buys nothing**: per-token throughput improves as depth predicts (prefill +22.3%, decode +28.7% against a 17.4% linear expectation) but 3 interleaved reps give **1227 → 1227 ms** end-to-end, because retrieval is a fixed per-turn cost above five tools and a degraded model emits more tokens; the blob shrinks 13.74 → 12.49 MB (−9.1%). Verdict: 3.7 accuracy points for 9% of a 14 MB file and no latency win. Two errors caught by `recheck.py` on first run and two measurement artefacts logged, including a k=5 wall-clock that said the *pruned* model was slower (output-length confound, fixed by reporting tok/s). |
...
### `needle-quantization/` — how much precision Needle 2 actually needs

The third and last of the "can this 45M model be made smaller" questions, and
the one where the premise had to be checked before the experiment: the model
already ships at 2 bits. So the sweep runs in both directions from there.

The answer is a flat line. Ten arms, eight of them within three queries of each
other, none significantly apart, over blobs ranging from 12.36 MB to 23.17 MB.
Precision above 2 bits is not what this model is spending its accuracy on, and
the exporter offers nothing below 2 bits that is actually smaller — the ternary
option stores 2-bit crumbs, so it trades a codebook level for no bytes at all,
and loses 39 points doing it.

What is left is small and real: the 4-bit protection Cactus puts on the
embedding and the MHC lanes is not load-bearing on this task. Dropping it is 10%
off the file at p=1.00. That is the whole harvest, and it is more than pruning
offered.

### `needle-layer-pruning/` — deleting depth from a shipped 45M checkpoint

The mirror of `needle-depth-growth/`, and the cheaper question: pruning needs no
training, so it can simply be measured. Twenty-four positions of a four-layer
cut, plus eight- and twelve-layer cuts, all scored on the same 62 queries the
two sibling experiments use.

The result is a wall. One position in twenty-four is statistically
indistinguishable from the unpruned model; everything else costs between 11 and
61 points, and five positions leave a model that answers nothing at all. A 45M
model trained to a 27-layer budget does not carry the depth redundancy that
large decoders do.

Two things surprised me enough to be worth the writeup on their own. The
redundant block is in the *middle* of the stack — the deep end, where the
large-model literature says to cut, is the most damaging place to touch. And the
standard angular-distance heuristic for finding the prunable block correlates
with the truth at 0.06, so it could not have saved a single one of the
twenty-four runs.

The deployment answer is short: the one surviving cut trades 3.7 accuracy points
for 9% of a 14 MB file and no latency improvement at all.
