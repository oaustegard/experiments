...
depth 1, summed over three repos, with no consistent sign), *and* it would not
have worked if used, because `git log --diff-filter=D` sees only the grafted
